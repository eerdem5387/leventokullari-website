...
export default async function BlogTagPage({ params }: BlogTagPageProps) {
  ...
  // Etiketi ve yazılarını getir
  const [tag, posts] = await Promise.all([
    prisma.blogTag.findUnique({
      where: { slug }
    }),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {posts.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Bu etikette henüz yazı yok</h3>
            <p className="text-gray-500">Yakında bu etikette güncel içeriklerle burada olacağız.</p>
          </div>
        )}
      </div>
    </div>
